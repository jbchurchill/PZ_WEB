  ...
  function sumSelectedParcelInfo(event) {
    ...
    arrayUtil.forEach(event.features, function (feature) {
      ...
      if (parcelIndex == -1) { // (arrStructNum.indexOf(arrParcelData) == -1) { // (parcelIndex === -1) { // current value is not in the Array
        ...
        if (plusOrMinus == 1) {
          // strAddresses += feature.attributes.ADDRESS + "<br />";
          arrStructNum[arrStructNum.length] = parcelValue; // arrParcelData; // new Array ([arrParcelData[0], arrParcelData[1], arrParcelData[2]]);
          console.log("Additional Value Added: " + arrStructNum.length + "th Value: " + parcelValue);
          ////////arrStructNum.push(arrParcelData);
          // pointSum += 1;
        } else if (plusOrMinus == 0) {
          // DO NOTHING ... Attempt to remove a parcel that is not in the array
        }
        // END SEPARATE IF STATEMENT        
      } else { // Array has at least one value and the present value is one of them
        // START SEPARATE IF STATEMENT
        console.log("HOLD EVERYTHING ... CURRENT VALUE IS ALREADY IN THE ARRAY WOOOOOOT");
        if (plusOrMinus == 0) {
          // strAddresses = strAddresses.replace(feature.attributes.ADDRESS + "<br \/>", '');
      ...
      */
      ...
      // parcelSum += 1;
      // console.log("Index: " + parcelIndex);
      // console.log("Array: " + arrStructNum);
      /*
      if (arrStructNum.length == 0) {
        // push the 3 element arrParcelData Array onto the end of the arrStructNum array.
        arrStructNum[0] = parcelValue; //[arrParcelData]; // new Array (arrParcelData[0], arrParcelData[1], arrParcelData[2]);
        // arrStructNum.push(new Array (arrParcelData[0], arrParcelData[1], arrParcelData[2]));
